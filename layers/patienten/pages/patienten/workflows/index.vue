  ...
  <div class="p-6 max-w-5xl">
    ...
    <div v-if="workflows.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
      <div
        ...
        @click="navigateTo(`/patienten/workflows/${wf.id}`)"
      >
        <div class="flex items-start justify-between mb-2">
          <p class="text-sm font-medium text-dental-blue-0 truncate">{{ wf.name }}</p>
          <button class="flex-shrink-0 ml-2" @click.stop="toggleActive(wf)">
            <span class="inline-block w-8 h-[18px] rounded-full relative transition-colors" :class="wf.is_active ? 'bg-dental-blue-0' : 'bg-dental-blue--5'">
              <span class="absolute top-[2px] w-[14px] h-[14px] rounded-full bg-white shadow transition-transform" :class="wf.is_active ? 'left-[16px]' : 'left-[2px]'" />
            </span>
          </button>
        </div>
    ...
    <div class="mt-8">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <div v-for="(tpl, i) in templates" :key="i" class="bg-soft-concrete--1 rounded-lg border border-dashed border-dental-blue--4 p-4">
          ...
          <button class="px-2.5 py-1 text-[10px] font-medium text-dental-blue-0 border border-dental-blue-0 hover:bg-dental-blue-0/5 rounded-lg" @click="useTemplate(tpl)">
            Verwenden
          </button>
        </div>
      </div>
...
const createNew = () => {
  ...
  navigateTo(`/patienten/workflows/${wf.id}`)
}
